...
description:
  - Engines refers to any device that is deployed and managed by the Forcepoint
    NGFW Management Center. More specifically, an engine can be physical or virtual,
...
def to_yaml(engine):
    if 'single_fw' in engine.type or 'cluster' in engine.type:
        ...
        raise ValueError('Only single firewall and firewall cluster types are '
            'currently supported.')


class EngineFacts(ForcepointModuleBase):
    def __init__(self):
